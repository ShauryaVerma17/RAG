...
def getData(url):
    ...
    if downloaded is not None:
        result = tf.extract(downloaded, target_language="en")
        return result
...
st.markdown("""
1. Paste the link to the webpage
2. Give the app some time to look at the document
3. Ask any question you want about the page
""")

with streamlit_analytics.track():
    # URL input for document ingestion
    st.session_state['url'] = st.text_input("Enter URL to crawl and ingest the webpage:", value=st.session_state['url'])
